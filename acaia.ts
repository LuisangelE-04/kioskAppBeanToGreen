...
async function connectScale(): Promise<void> {
  const parser = new AcaiaParser();
  ...
  try {
    ...
    const service = await server.getPrimaryService(READ_UUID);
    const readChar = await service.getCharacteristic(READ_UUID);
    const writeService = await server.getPrimaryService(WRITE_UUID);
    const writeChar = await writeService.getCharacteristic(WRITE_UUID);

    console.log("Connected to Acaia scale");

    readChar.addEventListener("characteristicvaluechanged", (event) => {
      const target = event.target as BluetoothRemoteGATTCharacteristic;
      const data = new Uint8Array(target.value!.buffer);
      for (const grams of parser.feed(data)) {
        if (lastWeight === null || Math.abs(grams - lastWeight) >= 0.01) {
    ...
    await writeChar.writeValue(encodeId());
    await new Promise((r) => setTimeout(r, 200));
    await writeChar.writeValue(encodeNotificationRequest());

    // Heartbeat every 5 seconds
    setInterval(async () => {
      await writeChar.writeValue(encodeId());
      await writeChar.writeValue(encodeHeartbeat());
    }, 5000);
  } catch (error) {
